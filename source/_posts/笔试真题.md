...
- `download`:导致浏览器将链接的URL视为下载资源。可以使用或不使用`filename`值
  - 如果没有指定值,浏览器会从多个来源决定文件名和拓展名
    - `Content-Disposition`HTTP标头
    - URL路径的最后一段
    - 媒体类型.来自`Content-Type`标头,`data:URL`的开头,或`blob:URL`的`Blob.type`
  - `filename`:决定文件名的值.`/`和`\`会被转化为下划线(`_`).文件系统可能会阻止文件名中的其他的字符,因此浏览器会在必要时适当调整文件名.
...
- `href`:超链接所指向的URL.链接不限于基于HTTP的URL--它们可以使用浏览器支持的任何URL协议
  - 使用文档片段链接到页面的某一段
  - 使用文本片段链接到某一段文字
  - 使用媒体片段链接到某个媒体文件
  - 使用`tel:`URL链接到一个电话号码
  - 使用`mailto:`URL链接到一个邮箱地址
  - 如果web浏览器不能支持其他URL协议,网站可以使用`registerProtocalHandler()`
- `hreflang`:该属性用于指定所链接到的文档的人类语言.其仅提供建议,并没有内置的功能.其允许的值与`全局的lang属性`一致.
- `ping`:包含一个以空格分隔的URL列表,当跟随超链接时,浏览器会发送带有正文`PING`的`POST`请求.通常用于跟踪.
- `referrerpolicy`:在跟随链接时,`referrer`需要发送多少内容
  - `no-referrer`：[`Referer`](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Headers/Referer) 标头将不会被发送。
  - `no-referrer-when-downgrade`：如果没有 [TLS](https://developer.mozilla.org/zh-CN/docs/Glossary/TLS)（[HTTPS](https://developer.mozilla.org/zh-CN/docs/Glossary/HTTPS)），[`Referer`](https://developer.mozilla.org/zh-CN/docs/Web/HTTP/Headers/Referer) 头将不会被发送到[源](https://developer.mozilla.org/zh-CN/docs/Glossary/Origin)上。
...
- `rel`该属性制定了目标对象到连接对象的关系.
- `target`该属性指定在何处显示链接的URL,作为浏览上下文的名称(标签,窗口或`<iframe>`).
  - `_self`:当前页面加载(默认)
  - `_blank`:通常在新标签页打开,但用户可以通过配置选择在新窗口打开.
  - `_parent`:当前浏览器环境的父级浏览上下文.如果没有父级框架,行为与`_selt`相同.
...
- `type`该属性指定在一个 [MIME 类型](https://developer.mozilla.org/zh-CN/docs/Glossary/MIME_type)链接目标的形式的媒体类型。没有内置的功能。
...
A. [HTML](https://developer.mozilla.org/zh-CN/docs/Web/HTML) **`<datalist>`** 元素包含了一组 [`<option>`](https://developer.mozilla.org/zh-CN/docs/Web/HTML/Element/option) 元素，这些元素表示其他表单控件可选值。是H5的新增元素.datalist提供了一些选项，但是也可以自己输入内容。select只能从提供的内容中进行选择。

B. **HTML 元素 `<optgroup>` **为[`<select>`](https://developer.mozilla.org/zh-CN/docs/Web/HTML/Element/select) 元素中的选项创建分组。

C. **HTML `<output>` 标签**表示计算或用户操作的结果。是H5新增属性.

D. **`<legend>`** [HTML](https://developer.mozilla.org/zh-CN/docs/Web/HTML) 元素表示其父元素 [`fieldset`](https://developer.mozilla.org/zh-CN/docs/Web/HTML/Element/fieldset) 内容的标题。
...
1. **`<img>`**：图像元素，内容是由 `src` 属性指定的图像文件。
2. **`<video>`**：视频元素，内容是由 `src` 属性指定的视频文件。
3. **`<audio>`**：音频元素，内容是由 `src` 属性指定的音频文件。
4. **`<object>`**：用于嵌入外部对象，如Flash、PDF等。
5. **`<iframe>`**：内联框架，用于嵌入另一个HTML文档。
6. **`<embed>`**：用于嵌入外部应用或插件。
7. **`<canvas>`**：用于绘制图形，内容由JavaScript生成。

特点

1. **固有尺寸**：置换元素通常有自己的固有尺寸，这些尺寸是由外部资源决定的。例如，图像的宽度和高度是由图像文件本身决定的。
2. **内容不受CSS控制**：置换元素的内容不由CSS直接控制，但可以通过CSS设置其尺寸和样式。
3. **布局影响**：置换元素在布局中占据一定的空间，其尺寸会影响周围的元素布局
...
[取值](https://developer.mozilla.org/zh-CN/docs/Web/CSS/background-attachment#取值)
...
- **标准盒模型**，也称为 `content-box`，在这个模型中，元素的 `width` 和 `height` 属性仅适用于元素的内容区域。任何 `padding` 和 `border` 都会增加元素的实际总宽度和高度。
- **IE盒模型** 或者说 **替代盒模型**，也可以称为 `border-box`，在这个模型中，元素的 `width` 和 `height` 包括了内容区域、`padding` 以及 `border`。这意味着无论 `padding` 和 `border` 的值是多少，元素的总宽度和高度都不会超过指定的 `width` 和 `height` 值。

可以通过 CSS 的 `box-sizing` 属性来选择使用哪种盒模型。例如，设置 `box-sizing: content-box;` 使用标准盒模型，而设置 `box-sizing: border-box;` 则使用 IE 盒模型或替代盒模型。
...
在非严格模式下运行,a被声明,b没有被声明直接赋值为5,所以被提升为全局变量.

而a使用var关键字声明,作用域在函数内部,所以全局访问a报错,b正常输出5.
...
>这里尤其要注意，使用setAttribute属性设置禁用时，无论第二个参数设置什么，都可以禁用，哪怕是`document.getElementById("button1").setAttribute('disabled','false')`也同样可以禁用。
 ...
 **含义**：先入先出算法是最简单的页面置换算法。当内存中没有空闲页面时，FIFO算法会选择最早进入内存的页面进行替换。这种方法简单易实现，但存在一个明显的缺点，即可能会导致“Belady’s anomaly”（贝拉迪异常），即增加分配给进程的页面数反而会导致缺页中断次数增加。

2. **LRU** (Least Recently Used)

**全称**：Least Recently Used
 **含义**：最近最少使用算法选择最近最久未被访问的页面进行替换。LRU算法假设最近一段时间内未被访问的页面在未来一段时间内也不会被访问。这种方法通常比FIFO更有效，因为它考虑了页面的使用频率和时间。然而，LRU算法的实现相对复杂，需要记录每个页面的访问时间或使用位。

3. **Clock** (时钟算法)

**全称**：Clock Algorithm
 **含义**：时钟算法是一种改进的FIFO算法，也称为第二次机会算法。它使用一个指针（类似于时钟的指针）遍历内存中的页面。当需要替换页面时，指针指向的第一个页面如果被标记为最近已使用（通常通过一个使用位来表示），则清除该标记并将指针移动到下一个页面，直到找到一个未被标记的页面进行替换。这样可以避免FIFO算法的一些缺点，但仍相对简单。

4. **LFU** (Least Frequently Used)

**全称**：Least Frequently Used
 **含义**：最不经常使用算法选择在一段时间内使用频率最低的页面进行替换。LFU算法假设使用频率低的页面在未来一段时间内也不太可能被使用。这种方法可以更好地反映页面的实际使用情况，但实现起来较为复杂，需要记录每个页面的使用次数。此外，LFU算法还需要定期更新页面的使用计数，以防止长时间运行的进程占据过多的内存。

### 总结

- **FIFO**：简单但可能存在“Belady’s anomaly”。
- **LRU**：考虑了页面的最近使用情况，通常更有效，但实现复杂。
- **Clock**：改进的FIFO，避免了一些FIFO的缺点，实现相对简单。
- **LFU**：考虑了页面的使用频率，通常更有效，但实现复杂。
...
稳定排序算法：鸡(基数排序)毛(冒泡排序)并(归并排序)插(插入排序)

## SQL语句

![](字节1709.png)

- `title like '%cv%'`：`%` 是SQL中的通配符，表示任意数量的任意字符。`%cv%` 表示 `title` 字段中包含 `cv` 的任何字符串。
- `and type=1`：表示 `type` 字段的值必须等于 1。

其他选项的错误之处：

- **A**：`title = 'cv'` 只会匹配 `title` 完全等于 `cv` 的记录，而不是包含 `cv` 的记录。
- **C**：`title like '*cv'` 使用了错误的通配符 `*`，SQL 中应该使用 `%`。
- **D**：`title ='*cv*'` 同样使用了错误的通配符 `*`，并且末尾的单引号不匹配（最后一个引号是中文引号）。
...
Nim游戏是一种经典的组合博弈游戏，通常由两名玩家轮流从若干堆物品中取走一定数量的物品，目标是迫使对方无法继续取物。Nim游戏的具体规则和策略非常有趣，下面我们来详细介绍一下。

**基本规则**

1. **初始状态**：游戏开始时，有若干堆物品（通常用石子表示），每堆有一定的数量。
2. **轮流取物**：两名玩家轮流取物，每次可以从任意一堆中取走至少1个物品，最多可以取走这一堆的所有物品。
3. **胜利条件**：最后一个取走物品的玩家获胜，即迫使对方无法继续取物的玩家获胜。

**举例**

假设初始状态有三堆石子，分别为 `(3, 4, 5)`。玩家A和玩家B轮流取物，每次可以从任意一堆中取走1到该堆剩余数量的石子。

Nim**游戏的核心策略**

Nim游戏的关键在于理解“Nim和”（Nim-sum），即所有堆的物品数量的二进制异或（XOR）结果。

Nim**和的计算**

1. **计算每堆物品数量的二进制表示**。
2. **对所有堆的二进制表示进行按位异或运算**。
...
1. **Nim和为0**：如果当前状态的Nim和为0，那么当前玩家处于必败态。无论他怎么取，对手总可以通过适当的取法使得Nim和再次变为0。
2. **Nim和不为0**：如果当前状态的Nim和不为0，那么当前玩家处于必胜态。他可以通过适当的取法使得Nim和变为0，从而将对手置于必败态。

**如何操作**

1. **找到一个堆**：找到一个堆，使得从这个堆中取走一定数量的物品后，新的Nim和为0。
2. **计算取法**：设当前Nim和为 `S`，当前堆的数量为 `x`，需要取走的数量为 `y`，则 `y = x XOR S`。

**示例**

假设初始状态为 `(3, 4, 5)`，Nim和为2：

- 选择堆 `5`，因为 `5 XOR 2 = 7`，但7大于5，所以不可行。
- 选择堆 `4`，因为 `4 XOR 2 = 6`，但6大于4，所以不可行。
- 选择堆 `3`，因为 `3 XOR 2 = 1`，可行。

所以，玩家可以取走2个石子，使得堆 `3` 变为 `1`，新的状态为 `(1, 4, 5)`，Nim和为0。

**总结**

- **Nim和为0**：当前玩家必败。
- **Nim和不为0**：当前玩家必胜，通过适当取法使得Nim和变为0。

Nim游戏不仅在理论上有重要意义，也在实际编程和算法竞赛中经常出现。理解Nim游戏的策略可以帮助你在各种博弈问题中取得优势。希望这些介绍对你有所帮助！
...
void (async function () {
  ...
  for (let i = 0; i < n; i++) {
    // 解构子串开始下标和长度
    const [p, l] = (await readline()).split(' ').map(Number);
    // 截取子串并翻转
    let sub = s
      .slice(p, p + l)
      .split('')
      ...
      .join('');
    // 将翻转后的子串插入原来子串位置正后方
    s = s.slice(0, p + l) + sub + s.slice(p + l);
  }
  // 输出结果
  console.log(s);
})();
...
void (async function () {
    // Write your code here
    const [n, s, L] = (await readline()).split(" ").map(Number);
    // 每张CD内的歌曲数
    let count = Math.trunc((L + 1) / (s + 1));
    // 如果count正好为13的整数倍，则减一
    if (count % 13 === 0) count--;
    // 需要的CD张数
    let result = Math.ceil(n / count);
    // 如果只需要一张CD且歌曲数为13整数倍或count-1为13整数倍且最后一个是13倍数
    if(result===1&&n%13===0||((count-1)%13===0&&(n%count)&&((n%count)%13===0))) result++;
    console.log(result);
})();

...
**定义**:图G是由顶点集V和边集E组成，记为G=(V,E)，其中V(G)表示图G中顶点的有限非空集合；E(G)表示图G中各顶点之间的关系(边)集合。用$|V|$表示图G中顶点的个数，也称为{%label 图G的阶 green%} ．用$|E|$表示图G中边的条数.

>**注意**
>
>线性表可以是空表，树可以是空树，但图不可以是空图。即图的顶点个数不能为0，但是边的个数可以为0.

若E是无向边(简称为边)的有限集合时,则图G为无向图.边是顶点的无序对.

- 顶点v的度是指依附于该点的边的条数，记为TD(V)
- 顶点v到顶点w有路径存在,则称v和w是连通的
- 若图G中任意两个顶点之间是连通的,则称G为{%label 连通图 green%},否则为非连通图
...
**排序(sort)**:就是重新排列表中的元素，使其关键字有序。

**算法的稳定性**：关键字相等的元素排序后相对位置不变。
...
| 算法         | 思想                                                         | 稳定性 | 时间复杂度                                                 | 空间复杂度                    | 备注                                                         |
| ------------ | ------------------------------------------------------------ | ------ | ---------------------------------------------------------- | ----------------------------- | ------------------------------------------------------------ |
| 插入排序     | 每次将一个待排序的记录按照其关键字的大小插入到前面已经排好序的子序列中，直到全部记录插入完成。 | √      | $O(n^2)$                                                   | $O(1)$                        | 当使用顺序存储结构时，插入前的查找可以优化为折半查找。       |
| 希尔排序     | 基于如果排序表基本有序，则插入排序效率较高。所以先追求表中元素部分有序，再逼近全局有序。<br/>先将待排序表分割成若干形如$L[i,i+d,i+2d,...,i+kd]$的特殊子表,对各个子表分别进行直接插入排序.缩小增量d,重复上述过程,直到d=1为止. | ×      | $O(n^2)$                                                   | $O(1)$                        |                                                              |
| 冒泡排序     | 依次比较两个元素,如果不满足大小关系,则交换,两个方向都可以.   | √      | $O(n^2)$                                                   | $O(1)$                        | 属于交换排序,如果一次遍历后没有发生交换,说明表已经有序.      |
| 快速排序     | 在待排序表$L[1...n]$中任取一个元素pivot作为枢轴(基准,通常取首元素),通过一趟排序将待排序表划分为独立的两部分,使得左侧表中所有元素都小于枢轴,右侧表中所有元素都大于等于枢轴.则枢轴的位置已经正确.分别递归地对两个子表重复上述过程,直至每部分内只有一个元素或空为止,即所有元素放在了其最终位置上. | ×      | 最好$O(nlogn)$<br />最坏$O(n^2)$                           | 最好$O(logn)$<br />最坏$O(n)$ | 也是交换排序.<br />如果初始序列有序或逆序,则快速排序性能最差 |
| 简单选择排序 | 每一趟在待排序元素中选取关键字最小的元素加入有序子序列       | ×      | $O(n^2)$                                                   | $O(1)$                        | 属于选择排序                                                 |
| 堆排序       | 完全二叉树如果满足父结点大于等于左右子节点,则将该完全二叉树层次遍历得到的即为大根堆(大顶堆) | ×      | $建堆O(n),堆排序(nlogn)$<br />总的$O(n)+O(nlogn)=O(nlogn)$ | $O(1)$                        | 属于选择排序<br />建立堆,小元素不断下坠<br />对堆进行排序,将根节点与最后一个元素交换,再将前面的元素重新建立大根堆 |
| 归并排序     | 将两个或多个已经有序的序列合并为一个                         | √      | $O(nlogn)$                                                 | O(n)                          |                                                              |
| 基数排序     |                                                              | √      | $O(d(n+r))$                                                | $O(r)$                        | 将关键字拆分为d位,每位可能有r个取值                          |
| 外部排序     |                                                              |        |                                                            |                               |                                                              |
